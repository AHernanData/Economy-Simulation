    ...
    def get_job(self):
        ...
        for j in range(len(n)):
            if j == 6:
                ...
                break
            product = random.randrange(0,len(company.company_list)
            business = random.randrange(0,len(company.company_list[product]))
            if company.company_list[product][business].hire(self) == True:
                self.employed = True
                break
...
def start_simulation_env(population,wealthy=.20,middle=.60,poor=.20):
    wealthy_pop = int(wealthy*population)
    middle_pop = int(middle*population)
    poor_pop = int(poor*population)
    ...
    pop_list = citizen.list_r()
    for i in range(len(pop_list)):
        pop_list[i].make_company()

def first_day():
    cit_list = citizen.list_r()
    comp_list = company.list_r()
    for i in range(len(cit_list)):
        cit_list[i].get_food()
        if cit_list[i].employed == False:
            cit_list[i].get_job()
    for i in range(len(comp_list)):
        comp_list[i].month_end()
    
#--------------------------------Test Commands-------------------------------#

start_simulation_env(50,.05,.50,.45)
first_day()