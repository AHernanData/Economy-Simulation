...
class citizen:
    ...
    def __init__(self, money, happiness,config=1):
        ...
        self.employed = False
        self.log = [['DAY {}\n'.format(simulation.Day)],[self.name, 'started with ${} and {} happiness'.format(self.money, self.happiness)]]
...
class company:
    ...
    def month_end(self):
        ...
        for i in range(len(self.employees)-1):
            ...
            self.employees[i+1].log.append(['Paid ${}.'.format(employee_income)])
...
class simulation:
    ...
    def one_day():
        ...
        simulation.Day += 1
        for i in range(len(citizen.citizen_list)):
            citizen.citizen_list[i].log.append(['DAY {}\n\n'.format(simulation.Day)])
