...
class company:
    ...
    def pay_manufacturer(self,industrial_cost):
        if self.product != 1:
            try:
                choice = random.choices(manufacturing_c.manufacturing_c_list,weights=wants.company_influence[1])
    ...
    def evaluate_financial_standing(self,wage_data):
        ...
        if self.positions_available + 5 < employee_potential:
            self.expand_ticker = 0
            self.positions_available = max(int(employee_potential),0)
        elif self.expand_ticker >= 5:
            self.expand_ticker = 0
            self.positions_available += 1
        elif self.positions_available < employee_potential:
            self.expand_ticker += 1
        elif self.positions_available < len(self.employees) or self.employees == []:
            self.expand_ticker = 0
            self.lay_off()
...
class wants:
    ...
    def set_weights(): 
        wants.company_influence = []                                                         #sets up a list containing the "influence" weight each company has due to their worth. indices in list line up with the indicies in the overall company list
