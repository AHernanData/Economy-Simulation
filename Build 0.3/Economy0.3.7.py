...
class citizen:
    ...
    def __init__(self, money, happiness,config=1):
        ...
        self.log = [['MONTH {}'.format(simulation.Month)],[self.name, 'started with ${} and {} happiness'.format(self.money, self.happiness)]]
    ...
    def end_simulation():
        ...
        for i in citizen.citizen_list:
            if i.money > y:
                y = i.money  
                richest_guy = i
        print(richest_guy.name, "is the richest with ${}.".format(round(y,2)))
        global richest
        richest = richest_guy
        y = 0
        global c
...
simulation.start_simulation_env(population=300,wealthy=30,middle=36,poor=34)
for x in range(70):
